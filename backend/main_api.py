...
async def create_book(book_data: BookCreate, database: Session = Depends(get_db)) -> Book:
    ...
    db_book = Book(
        time=book_data.time,        release=book_data.release,        stock=book_data.stock,        price=book_data.price,        title=book_data.title,        pages=book_data.pages        )
...
async def bulk_create_book(items: list[BookCreate], database: Session = Depends(get_db)) -> dict:
    ...
    for idx, item_data in enumerate(items):
        try:
            ...
            db_book = Book(
                time=item_data.time,                release=item_data.release,                stock=item_data.stock,                price=item_data.price,                title=item_data.title,                pages=item_data.pages            )
            database.add(db_book)
